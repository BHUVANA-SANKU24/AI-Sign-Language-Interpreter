...
(function () {
    ...
    async function grammarPolish(rawText) {
        const GEMINI_KEY = 'AIzaSyB1nSraezJDmyvqEpNWg81GbOSaFK6jri4'; // replace with real key or leave for demo
        // Demo mode: simple rule-based cleanup when no key
        if (GEMINI_KEY === 'AIzaSyB1nSraezJDmyvqEpNWg81GbOSaFK6jri4') {
            return demoGrammarFix(rawText);
        }
        try {
            const resp = await fetch(
                `https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=${GEMINI_KEY}`,
                {
                    ...
                    body: JSON.stringify({
                        contents: [{
                            parts: [{
                                text: `Convert this ASL-style telegraphic text into natural English. Only return the corrected sentence, nothing else.\n\nInput: "${rawText}"`
                            }]
                        }]
                    }),
                }
    ...
    function demoGrammarFix(text) {
        // Simple demo: capitalize words, add period, fix "HELLO AMERICA TODAY TALK" → "Hello America, today I talk."
        const words = text.toLowerCase().split(/\s+/).map(w => w.charAt(0).toUpperCase() + w.slice(1));
        return words.join(' ') + '.';
    }
    ...
    function showPolishResult(orig, polished) {
        ...
        document.getElementById('usePolishedBtn').onclick = () => {
            document.getElementById('textInput').value = polished;
            document.getElementById('charCount').textContent = polished.length;
    ...
    function bindMicControls() {
        ...
        if (!MicAPI) { micBtn.style.opacity = '0.4'; micBtn.title = 'Not supported (use Chrome)'; return; }

        micBtn.addEventListener('click', () => {
            if (isMicActive) { speechRecognition?.stop(); return; }
            ...
            speechRecognition.onerror = (e) => { showToast('Mic: ' + e.error, 'error'); stopMic(); };
